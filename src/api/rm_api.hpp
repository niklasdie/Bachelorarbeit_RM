...
void init_rm_api(const void *shm_address, const size_t shm_size, const size_t size_of_struct) {
    shm_ = (shm_o *) ((char *) shm_address + size_of_struct);
    udp_sender_ = (udp_sender*) ((char *) shm_address + size_of_struct + sizeof(shm_o));

    udp_sender_->set_shm(shm_);

    BOOST_LOG_TRIVIAL(debug) << (char*) shm_address - (char*) shm_;
    BOOST_LOG_TRIVIAL(debug) << (char*) shm_address - (char*) udp_sender_;
    BOOST_LOG_TRIVIAL(debug) << (char*) shm_ - (char*) udp_sender_;
...
void rm_in_sl(const void *src, const size_t length) {
    if ((*shm_).set_data(src, length)) {
        (*udp_sender_).send_data((*shm_).get_address(), length);
    }
}
...
void rm_in_ts(const void *src) {
    if ((*shm_).set_data(src, sizeof(T))) {
        (*udp_sender_).send_data((*shm_).get_address(), sizeof(T));
    }
}
